...
class CategoryController extends AdminController {
    ...
    public function index(){
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status,model');
    ...
    public function removeall(){
    	...
    	$idlist =  implode(',', $ids);
    	//判断该分类下有没有内容
    	$document_list = M('Document')->where(array('category_id'=>array('in' , $idlist)))->field('id')->select();
    	if(!empty($document_list)){
    		foreach ($document_list as $k => $v){
    			$arrdo[] = $v['id'];
    		}
    	...
    	if(M('Category')->where(array('id' => array('in' , $idlist)))->delete()){
    		//记录行为
    		action_log('update_category', 'category', $idlist, UID);
    		$this->success('删除分类成功');
    	}else{
    		$this->error('删除分类失败！');
    	}
